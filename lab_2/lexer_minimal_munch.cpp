 ...
 * Note 	: It uses the power of DFA + backtracking
 */
...
void dfa_simulate(struct dfa *df)
{
	...
	string prev_accept_string = "";
	string prev_accept_state = "";
	string buffer="";
	...
	while(1)
	{
			...
			if(nc == -1)
			{
				if(dfa_isaccept_state(df,cur_state) == 1)
				{
					...
					cout<<"Accepting in break:"<<buffer<<"###"<<endl;
				}else
				{
					//Error
					cout<<"Error<<" << buffer;
			...
			}else if(next_state != "")		
			{
					...
					if(dfa_isaccept_state(df,cur_state)==1 )
					{
							...
							if(prev_accept_string.size() < buffer.size())
							{
								prev_accept_string = buffer;
								prev_accept_state = cur_state;
							}					
							cur_state = next_state;
							buffer+= nc;
							continue;
			...
			}else if(next_state == "")
			{
					...
					{
						//Error
						if(prev_accept_string.size()!= 0)
						{
								token_class(prev_accept_state,prev_accept_string);
								fseek(input, -1*(buffer.size() - prev_accept_string.size() +1),SEEK_CUR);
								cur_state = starting_state;
								buffer= "";
								prev_accept_string = "";
								prev_accept_state = "";
